...
if config.model == "cem" or config.model == "escm":
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, inputs, mask=None, parents=None):
        x = inputs
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        #print(f"Multi: kv_input_depth: {kv_input_depth}")
        if kv_input_depth is None:
            kv_input_depth = input_depth
        # Key and query depth will be same
        self.query_linear = nn.Linear(input_depth, total_key_depth, bias=False)
        self.key_linear = nn.Linear(kv_input_depth, total_key_depth, bias=False)
        self.value_linear = nn.Linear(kv_input_depth, total_value_depth, bias=False)
...
def share_embedding(vocab, pretrain=True, emb_dim=config.emb_dim):
    embedding = Embeddings(vocab.n_words, emb_dim, padding_idx=config.PAD_idx)
...
def get_input_from_batch(batch):
    ...
    c_t_1 = torch.zeros((batch_size, 2 * config.hidden_dim))  # torch.Size([16, 600])
...
def evaluate(model, data, ty="valid", max_dec_step=30):
    ...
    if config.model != "empdg":
        pass
    for j, batch in pbar:
        if any([x in config.model for x in ["cem", "escm"]]):
...
class MultiHeadGCN(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.query_scale = (total_key_depth // num_heads) ** -0.5  ## sqrt
        self.value_linear = nn.Linear(input_depth, input_depth, bias=False)
    ...
    def _split_heads(self, x):
        ...
        return x.view(
            shape[0], shape[1], self.num_heads, shape[2] // self.num_heads
        ).permute(0, 2, 1, 3) # bz, num_head, src_len, dim
    ...
    def _split_kv_heads(self, x):
        ...
        if len(x.shape) != 4:
            raise ValueError("x must have rank 4")
        shape = x.shape # bz, src_len, src_len, dim
        #print(f"_split_heads: {shape}")
        return x.view(
            shape[0], shape[1], shape[2], self.num_heads, shape[3] // self.num_heads
        ).permute(0, 3, 1, 2, 4) # bz, num_head, src_len, src_len, dim
    ...
    def _merge_heads(self, x):
        ...
        shape = x.shape
        return (
            x.permute(0, 2, 1, 3) # (bz, num_head, src_len, dim) -> (bz, src_len, num_head, dim)
            .contiguous()
            .view(shape[0], shape[2], shape[3] * self.num_heads) # (bz, src_len, num_head, dim) -> (bz, src_len, total_key_depth)
        )

    def forward(self, queries, keys, values, mask, adjacency_matrix):

        values = self.value_linear(values)
        # Split into multiple heads
        queries = self._split_kv_heads(queries)
        keys = self._split_kv_heads(keys)  
        # (bz, num_head, src_len, dim)
        values = self._split_heads(values)

        # Combine queries and keys
        logits = torch.sum(torch.mul(queries, keys), dim=-1) # bz, num_head, src_len, src_len
        adjacency_matrix = adjacency_matrix.unsqueeze(1) 
        logits = logits.masked_fill(adjacency_matrix, -1e18)
