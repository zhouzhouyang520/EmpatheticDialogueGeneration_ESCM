...
from src.models.ESCM.model import ESCM
...
converted_name_dict = {"trs": "TRS", 
                        ...
                        "cem": "CEM", 
                        "escm": "ESCM"}
def make_model(vocab, dep_tree_vocab, dec_num):
    is_eval = config.test
    if config.model == "trs":
        model = Transformer(
            vocab,
    ...
    if config.model == "multi-trs":
        ...
        model = CEM(
            ...
            is_eval=is_eval,
            model_file_path=config.model_path if is_eval else None,
            dep_tree_vocab=dep_tree_vocab, 
        )
    elif config.model == "escm":
        model = ESCM(
            vocab,
            decoder_number=dec_num,
            is_eval=is_eval,
    ...
    try:
        ...
        for n_iter in tqdm(range(1000000)):
            ...
            if (n_iter + 1) % check_iter == 0:
                ...
                model.is_eval = False
                #if ppl_val <= best_ppl:
                if ppl_val <= best_ppl or (ppl_val < 43 and acc_val > 0.385):
                    best_ppl = ppl_val
                    patient = 0
...
def test_model_list(vocab, dep_tree_vocab, dec_num, test_set):
    base_dir = "save/test/"
    converted_name = converted_name_dict[config.model] 
    models = get_filenamea(base_dir, converted_name)
    ...
    with open(result_path, "a") as f:
        for model_name in models:
            ...
            model = make_model(vocab, dep_tree_vocab, dec_num)
...
def main():
    ...
    train_set, dev_set, test_set, vocab, dep_tree_vocab, dec_num = prepare_data_seq(
        batch_size=config.batch_size
    )

    if config.test:
        test_model_list(vocab, dep_tree_vocab, dec_num, test_set)
    else:
        model = make_model(vocab, dep_tree_vocab, dec_num)
